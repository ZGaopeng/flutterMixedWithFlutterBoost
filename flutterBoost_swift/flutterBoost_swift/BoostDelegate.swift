...
class BoostDelegate: NSObject, FlutterBoostDelegate {
    ...
    func pushNativeRoute(_ pageName: String!, arguments: [AnyHashable : Any]!) {
        ...
        if isPresent {
            self.navigationController?.present(targetViewController, animated: isAnimated)
        } else {
    ...
    func popRoute(_ options: FlutterBoostRouteOptions!) {
        ...
        if let vc = self.navigationController?.presentedViewController as? FBFlutterViewContainer,vc.uniqueIDString() == options.uniqueId{
            ...
            if vc.modalPresentationStyle == .overFullScreen {
                // 这里手动beginAppearanceTransition 触发页面声明周期
        ...
        if let onPageFinished = resultTable[options.pageName] {
